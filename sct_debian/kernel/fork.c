...
void send_ptree_info(int pid, int ppid, char *pinfo)
{
	char sendchar[PACKET_STR_SIZE];
	int len;

	len = snprintf(sendchar, PACKET_STR_SIZE, "%d%c%d%c%s", pid,
		       SCI_SEPCHAR, ppid, SCI_SEPCHAR, pinfo);
	sci_send(sendchar, len, p_sci_cs2);
}
