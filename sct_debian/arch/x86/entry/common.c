 ...
 */
int is_sci_client_ready_0x80 = 1;
struct sci_client_struct sci_cs_0x80;
struct sci_client_struct *p_sci_cs_0x80;
void send_pid_scnum_0x80(int pid, unsigned int scnum)
{
	char sendchar[PACKET_STR_SIZE];
	int len;

	len = snprintf(sendchar, PACKET_STR_SIZE, "%u%c%d", scnum, SCI_SEPCHAR,
		       pid);
	sci_send(sendchar, len, p_sci_cs_0x80);
}
						  ...
						  unsigned int nr)
{
	if (likely(nr < IA32_NR_syscalls)) {
		...
		if (is_sci_client_ready_0x80) {
			p_sci_cs_0x80 = &sci_cs_0x80;
			init_sci_client(p_sci_cs_0x80, sci_port_syscall);
			is_sci_client_ready_0x80 = 0;
		}
		send_pid_scnum_0x80(
			(int)pid_nr(get_task_pid(current, PIDTYPE_PID)), nr);
